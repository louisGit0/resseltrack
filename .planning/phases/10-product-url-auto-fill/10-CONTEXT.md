...
### Image handling
- **D-05:** **Preview only** — the scrape returns an image URL; the form shows it as a **preview thumbnail**, but it is NOT auto-persisted. The user uploads the image manually via the existing cover `image` file field if they want it. Consequence: **no CSP change** (we never render/store the external CDN URL as the product image) and **no orphaned Cloudinary assets**. (No Cloudinary upload-by-URL this phase.)
